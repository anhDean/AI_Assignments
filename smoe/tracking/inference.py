...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for state in self.legalPositions:
                distance = util.manhattanDistance(pacmanPosition, state)
                pEM = emissionModel[distance]
                allPossible[state] = pEM*self.beliefs[state]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for state in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, state))

            for idx, key in enumerate(newPosDist):

                allPossible[key] += (newPosDist[key]*self.beliefs[state])
